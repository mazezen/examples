...
func (h *Handler) CreatePost(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&post); err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("invalid request body: %s", err.Error())})
		return
	}
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Errorf("could not save post: %s", err.Error())})
	} else {
...
func (h *Handler) UpdatePost(c *gin.Context) {
	...
	if err = c.ShouldBindJSON(&post); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": fmt.Errorf("could not parse request body: %s", err.Error())})
		return
	}
	...
	if err != nil {
		switch err {
		case db.ErrNotRecord:
			c.JSON(http.StatusNotFound, gin.H{"error": fmt.Errorf("could not find post with id: %d", id)})
		default:
			h.Logger.Err(err).Msg("could not update post")
			c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Errorf("could not update post: %s", err.Error())})
		}
		return
	} else {
		c.JSON(http.StatusOK, gin.H{"success": post})
	}
}
...
// 	)
// 	if err != nil {
// 		h.Logger.Err(err).Msg("eleasticsearch error")
// 		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
// 		return
// 	}
// 	defer res.Body.Close()
